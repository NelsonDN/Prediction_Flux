...
def page_optimisation():
    ...
    with tabs[1]:
        ...
        # Extraire donnees de repartition
        # Les donnees Besoin sont en colonnes A-B, lignes 3-31
        # Les donnees Excedent sont en colonnes D-E, lignes 3-31
        
        st.markdown("**Repartition Besoin / Excedent**")
        
        # Reconstruire les dictionnaires depuis le fichier Excel
        besoin_dict = {}
        excedent_dict = {}
        
        # Lire les valeurs depuis le DataFrame
        for idx in range(2, len(df_optim)):  # Commencer a ligne 3 (index 2)
            # Colonne A (index 0) et B (index 1) pour Besoin
            if pd.notna(df_optim.iloc[idx, 0]) and pd.notna(df_optim.iloc[idx, 1]):
                agence = str(df_optim.iloc[idx, 0])
                montant = float(df_optim.iloc[idx, 1])
                if montant > 0:
                    besoin_dict[agence] = montant
            
            # Colonne D (index 3) et E (index 4) pour Excedent
            if pd.notna(df_optim.iloc[idx, 3]) and pd.notna(df_optim.iloc[idx, 4]):
                agence = str(df_optim.iloc[idx, 3])
                montant = float(df_optim.iloc[idx, 4])
                if montant > 0:
                    excedent_dict[agence] = montant
        ...
        st.markdown("""
        <div class="info-box">
        <b>Interpretation :</b> Cette matrice montre les montants transferes de chaque agence
        en excedent (lignes) vers chaque agence en besoin (colonnes). Les cellules vides
        indiquent aucun transfert. Les valeurs sont en Millions FCFA.
        </div>
        """, unsafe_allow_html=True)
        
        # Trouver le debut de la matrice (ligne avec "MATRICE D'ALLOCATION OPTIMALE")
        matrice_start = None
        for idx, row in df_optim.iterrows():
            if 'MATRICE' in str(row.iloc[0]).upper():
                matrice_start = idx + 2  # 2 lignes apres le titre
                break
        
        if matrice_start and matrice_start < len(df_optim):
            # Extraire la matrice
            df_matrice_raw = df_optim.iloc[matrice_start:]
            
            # Identifier les agences (headers et index)
            agences_besoin = []
            for col_idx in range(1, len(df_matrice_raw.columns)):
                val = df_matrice_raw.iloc[0, col_idx]
                if pd.notna(val) and str(val).startswith('0'):
                    agences_besoin.append(str(val))
            
            agences_excedent = []
            for row_idx in range(1, len(df_matrice_raw)):
                val = df_matrice_raw.iloc[row_idx, 0]
                if pd.notna(val) and str(val).startswith('0'):
                    agences_excedent.append(str(val))
            
            # Extraire valeurs
            if agences_besoin and agences_excedent:
                matrice_values = []
                for i, agence_exc in enumerate(agences_excedent, 1):
                    row_values = []
                    for j in range(1, len(agences_besoin) + 1):
                        val = df_matrice_raw.iloc[i, j]
                        row_values.append(float(val) if pd.notna(val) else 0.0)
                    matrice_values.append(row_values)
                ...
                fig_heatmap = go.Figure(data=go.Heatmap(
                    ...
                    y=agences_excedent,
                    colorscale='Blues',
                    text=[[f"{v:,.0f}" if v > 0 else "" for v in row] for row in matrice_values],
                    texttemplate="%{text}",
                    textfont={"size": 10},
                    hoverongaps=False,
                    hovertemplate='De: %{y}<br>Vers: %{x}<br>Montant: %{z:,.2f}M<extra></extra>'
                ))
                
                fig_heatmap.update_layout(
                    title="Matrice d'Allocation (Heatmap)",
                    xaxis_title="Agences en Besoin",
                    yaxis_title="Agences en Excedent",
                    height=600
                )
                ...
                nb_transactions = sum([1 for row in matrice_values for val in row if val > 0])
                
                col1, col2 = st.columns(2)
                
                with col1:
                    st.metric("Total Transferts", f"{total_transferts:,.2f} M")
    ...
    with tabs[3]:
        ...
        # Calculer cout total
        if matrice_start and agences_besoin and agences_excedent:
            cout_total = 0
            
            # Recuperer la matrice des couts
            df_couts_clean = df_couts.iloc[3:].copy()
            df_couts_clean.columns = ['Agence'] + list(df_couts.iloc[2, 1:])
            df_couts_clean = df_couts_clean.set_index('Agence')
            ...
            for i, agence_exc in enumerate(agences_excedent):
                for j, agence_bes in enumerate(agences_besoin):
                    ...
                    if montant > 0:
                        # ✅ GESTION DES CAS OÙ LA CLÉ N'EXISTE PAS
                        try:
                            cout_unitaire = df_couts_numeric.loc[agence_exc, agence_bes]
                            cout_total += montant * cout_unitaire
                        except KeyError:
                            st.warning(f"⚠️ Coût manquant pour {agence_exc} → {agence_bes}")
                            continue
    ...
    #                     cout_total += montant * cout_unitaire
            ...
            with col1:
                st.metric("Cout Total Optimal", f"{cout_total:,.2f} FCFA")
            ...
            with col3:
                if nb_transactions > 0:
                    cout_moyen = cout_total / nb_transactions
                    st.metric("Cout Moyen par Transaction", f"{cout_moyen:,.2f} FCFA")
        
        st.markdown("---")
        
        # Graphique flux BEAC
        df_entree_full = excel_data['Donnees entree']
        flux_beac = df_entree_full[df_entree_full['Code Agence'] == 'BEAC']['Flux transport optimal'].values
        
        if len(flux_beac) > 0:
            flux_beac_val = flux_beac[0]
            
            if flux_beac_val > 0:
                role = "Versement BEAC"
                color = '#FF6B6B'
            elif flux_beac_val < 0:
                role = "Retrait BEAC"
                color = '#4ECDC4'
            else:
                role = "Aucune Intervention"
                color = '#95A5A6'
            
            st.markdown(f"""
            <div class="info-box">
            <b>Role BEAC :</b> {role}<br>
            <b>Montant :</b> {abs(flux_beac_val):,.2f} Millions FCFA
            </div>
            """, unsafe_allow_html=True)
        ...
        if filepath_excel.exists():
            ...
            st.download_button(
                label=f"Telecharger Fichier Excel J+{jour_selectionne}",
                data=excel_bytes,

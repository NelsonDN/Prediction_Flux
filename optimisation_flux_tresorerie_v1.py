...
def get_stock_minimum_agence(agence):
    """
    Recupere le stock minimum reel d'une agence depuis le fichier CSV stocks_minimum_agences.csv
    
    Args:
        agence: Code agence (ex: '00001')
    
    Returns:
        Stock minimum en millions (valeur absolue)
    """
    filepath = 'stocks_minimum_agences.csv'
    
    if not Path(filepath).exists():
        raise FileNotFoundError(
            f"Fichier des stocks minimum introuvable: {filepath}\n"
            f"Veuillez executer calculer_stock_minimum.py d'abord"
        )
    
    # df_stocks = pd.read_csv(filepath)
    

    df_stocks = pd.read_csv(filepath, dtype={'Code_Agence': str}, encoding='utf-8-sig')
    ...
    if len(ligne_agence) == 0:
        raise ValueError(f"Stock minimum introuvable pour agence {agence}")
    
    # Prendre valeur absolue du stock minimum (P5)
    stock_min = abs(ligne_agence['Stock_Minimum_Millions'].values[0])
    
    return round(stock_min, 2)
